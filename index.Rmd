...
The selected model utilizes k-folds cross validation and a random forest machine learning algorithm.  **The 95% confidence interval for accurately identifying the correct "classe" is 99.48 to 99.94%.**   
...
# building simple table of run times and accuracy
...
I found that pre-processing does appear to be reducing complexity and runtime but at a cost to accuracy.  Extra k-folds does not appear to significantly increase performance given the extra processing time based on the other implementation decisions I have made.  
...
pred_rfnp_quiz <- predict(model_rfnp, testing)

quizTable <- enframe(pred_rfnp_quiz, name = "QuizQ", value = "Prediction")
